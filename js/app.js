...
PTE.App = {
  ...
  _renderAIFeedback(fb, q, type) {
    ...
    // Fluency Improvement section
    if (fb.fluencyAnalysis) {
      const fl = fb.fluencyAnalysis;
      const bandColor = fl.score >= 4 ? 'emerald' : fl.score === 3 ? 'cyan' : fl.score === 2 ? 'amber' : 'rose';

      html += `
      <div class="mb-4">
        <div class="flex items-center justify-between mb-3">
          <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wide">Fluency Improvement</h4>
          <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-${bandColor}-500/15 text-${bandColor}-400 border border-${bandColor}-500/20">Band ${fl.score}/5 — ${fl.bandLabel}</span>
        </div>
        ...
        <!-- Coaching Message -->
        <div class="bg-${bandColor}-500/5 rounded-lg p-3 border border-${bandColor}-500/10 mb-3">
          <p class="text-xs text-${bandColor}-300 leading-relaxed">${fl.coachingMessage}</p>
        </div>`;

      // Diagnostic Breakdown
      if (fl.diagnostics) {
        const diagEntries = Object.values(fl.diagnostics);
        html += `
        <div class="mb-3">
          <p class="text-[10px] text-zinc-500 uppercase tracking-wider mb-2 font-medium">Diagnostic Breakdown</p>
          <div class="space-y-2">
            ${diagEntries.map(d => {
              const dColor = d.score >= 80 ? 'emerald' : d.score >= 50 ? 'amber' : 'rose';
              return `
              <div>
                <div class="flex items-center justify-between mb-0.5">
                  <span class="text-[11px] text-zinc-400">${d.label}</span>
                  <span class="text-[10px] font-mono text-${dColor}-400">${d.score}%</span>
                </div>
                <div class="h-1.5 bg-white/[0.04] rounded-full overflow-hidden">
                  <div class="h-full rounded-full bg-${dColor}-500 transition-all" style="width:${d.score}%"></div>
                </div>
                <p class="text-[10px] text-zinc-600 mt-0.5">${d.desc}</p>
              </div>`;
            }).join('')}
          </div>
        </div>`;
      ...
      // Band Progression
      if (fl.progressionTip) {
        html += `
        <div class="bg-indigo-500/5 rounded-lg p-3 border border-indigo-500/10 mb-3">
          <p class="text-[10px] font-semibold text-indigo-400 uppercase tracking-wider mb-1">Next Level</p>
          <p class="text-xs text-indigo-300">${fl.progressionTip}</p>
        </div>`;
      }

      // Targeted Drills with links to Fluency Lab
      if (fl.targetedDrills && fl.targetedDrills.length > 0) {
        html += `
        <div class="mb-3">
          <p class="text-[10px] text-zinc-500 uppercase tracking-wider mb-2 font-medium">Recommended Fluency Drills</p>
          <div class="space-y-1.5">
            ${fl.targetedDrills.map(drill => `
              <a href="${drill.link}" onclick="setTimeout(()=>PTE.Fluency&&PTE.Fluency.startExercise('${drill.exercise}'),100)" class="flex items-center gap-3 p-2.5 rounded-lg bg-violet-500/5 border border-violet-500/10 hover:bg-violet-500/10 transition-all group cursor-pointer">
                <span class="text-lg">${drill.icon}</span>
                <div class="flex-1 min-w-0">
                  <p class="text-xs font-semibold text-violet-300 group-hover:text-violet-200">${drill.name}</p>
                  <p class="text-[10px] text-zinc-500">${drill.desc}</p>
                </div>
                <svg class="w-3.5 h-3.5 text-zinc-700 group-hover:text-violet-400 transition-colors shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
              </a>`).join('')}
          </div>
        </div>`;
      }
      ...
      // Pace Assessment
      html += `<p class="text-xs text-gray-400">${fl.paceAssessment}</p>`;
      ...
      <details class="mb-4">
        ...
        <div class="mt-2 space-y-2">
          ${fb.practiceExercises.map(ex => `
            <div class="bg-purple-500/10 rounded-lg p-3 border border-purple-500/20${ex.link ? ' cursor-pointer hover:bg-purple-500/15 transition-all' : ''}"${ex.link ? ` onclick="location.hash='${ex.link}'"` : ''}>
              <div class="flex items-center justify-between">
                <p class="text-xs font-semibold text-purple-300 mb-0.5">${ex.title}</p>
                ${ex.link ? '<svg class="w-3 h-3 text-purple-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>' : ''}
              </div>
