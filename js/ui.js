...
PTE.UI = {
  ...
  /**
   * Show accent selection modal
   */
  showAccentModal() {
    // Create modal overlay
    const modalId = 'accent-modal-' + Date.now();
    const existingModal = document.getElementById('accent-modal');
      ...
      <div class="glass rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-white/10">
          <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-white">Choose Practice Accent</h2>
            <button onclick="document.getElementById('${modalId}').remove()" class="w-8 h-8 rounded-lg hover:bg-white/10 flex items-center justify-center transition-colors">
              <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          </div>
          <p class="text-gray-400 mt-2">Select the accent you want to practice with. Different accents help you prepare for various PTE exam centers worldwide.</p>
        </div>
        <div class="p-6">
          <div id="accent-selector-${modalId}"></div>
          <div class="flex justify-end gap-3 mt-6">
            <button onclick="document.getElementById('${modalId}').remove()" class="px-4 py-2 text-gray-400 hover:text-white transition-colors">
              Cancel
            </button>
          </div>
        </div>
    ...
    // Render accent selector inside modal
    if (PTE.Accents) {
      PTE.Accents.renderAccentSelector(`accent-selector-${modalId}`);

      // Override the setAccent function to close modal after selection
      const originalSetAccent = PTE.Accents.setAccent;
      PTE.Accents.setAccent = function(accentId) {
        originalSetAccent.call(this, accentId);
        // Close modal
        setTimeout(() => {
          document.getElementById(modalId).remove();
          // Refresh the current page to show new accent
          const currentRoute = window.location.hash.slice(1) || '/';
          PTE.Router.navigate(currentRoute);
        }, 500);
      };
    }
