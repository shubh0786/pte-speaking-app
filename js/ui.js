...
PTE.UI = {
  ...
  navbar(activePage) {
    ...
    return `
    <nav class="glass border-b border-[var(--border)] sticky top-0 z-50" role="navigation" aria-label="Main navigation">
      <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="flex items-center justify-between h-14 sm:h-16">
          <a href="#/" class="flex items-center gap-2 sm:gap-3 group" aria-label="Crack PTE Home">
          ...
          <div class="flex items-center gap-2">
            ...
            ${userBtn}
            <button onclick="document.getElementById('${navId}').classList.toggle('hidden')" class="md:hidden w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white/10 transition-colors" aria-label="Open navigation menu" aria-expanded="false">
  ...
  spinner(text = 'Loading...') {
    ...
    <div class="flex flex-col items-center justify-center py-20" role="status" aria-live="polite">
      <div class="w-12 h-12 border-4 border-indigo-800 border-t-indigo-400 rounded-full animate-spin mb-4"></div>
  ...
  emptyState(icon, title, description, ctaText, ctaHref) {
    ...
    <div class="text-center py-16 animate-fadeIn">
      <div class="w-20 h-20 mx-auto mb-4 rounded-2xl bg-white/5 flex items-center justify-center">
        <span class="text-4xl animate-float">${icon}</span>
      </div>
      <h3 class="text-lg font-semibold text-white mb-2">${title}</h3>
      <p class="text-gray-400 text-sm max-w-md mx-auto mb-6">${description}</p>
  ...
  /**
   * Practice flow stepper — shows current phase: Setup → Listen → Prep → Record → Review
   */
  practiceFlowStepper(currentPhase, hasAudio) {
    ...
    return `
    <div class="practice-stepper" id="practice-stepper" role="progressbar" aria-label="Practice progress">
      ${steps.map((s, i) => {
        ...
        return `${line}<div class="step flex flex-col items-center">
          <div class="step-dot ${dotClass}" aria-current="${isActive ? 'step' : 'false'}">${icon}</div>
  ...
  /**
   * Skeleton loader card for page transitions
   */
  skeletonCards(count = 4) {
    return `<div class="grid sm:grid-cols-2 lg:grid-cols-${Math.min(count, 4)} gap-5">
      ${Array(count).fill(0).map(() => `
        <div class="skeleton-card">
          <div class="flex items-start justify-between mb-4">
            <div class="skeleton skeleton-circle" style="width:48px;height:48px"></div>
            <div class="skeleton" style="width:40px;height:24px;border-radius:12px"></div>
          </div>
          <div class="skeleton skeleton-text" style="width:70%"></div>
          <div class="skeleton skeleton-text" style="width:100%"></div>
          <div class="skeleton skeleton-text" style="width:50%"></div>
          <div class="skeleton" style="width:100%;height:6px;margin-top:16px;border-radius:3px"></div>
        </div>
      `).join('')}
    </div>`;
  },
   ...
   */
  kbdHint(key, label) {
    return `<span class="hidden md:inline-flex items-center gap-1.5 text-xs text-gray-500"><span class="kbd">${key}</span> ${label}</span>`;
  },
